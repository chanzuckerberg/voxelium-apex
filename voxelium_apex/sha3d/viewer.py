...
class Viewer:
    ...
    def __init__(self, summary, embed, structure_factors, device, title=None, bfac_step=20):
        ...
        self.spectral_indices = vxm.get_spectral_indices(self.ft_shape, device=self.device, center=False)
        ...
        for i in range(summary.basis.shape[0]):
            self.ref_basis_df[i] = vxm.dft(summary.basis[i].to("cpu"), real_in=True, center=False)
        ...
        self.hm_cm = vxm.get_default_cmap()
    ...
    def update_hm(self):
        ...
        blur = vxm.gaussian_blur(sharp, self.hm_sigma)
    ...
    def update_bfactor(self):
        ...
        grid = vxm.spectra_to_grid(profile, self.spectral_indices)

        for i in range(self.summary.basis.shape[0]):
            df = self.ref_basis_df[i].to(self.device) * grid
            base = vxm.idft(df, real_in=True, centered=False)
...
def main(args):
    ...
    if embed.size(1) > 2:
        ...
        if args.umap:
            ...
            print("Applying PCA...")
            embed = vxm.pca_dim_reduction(embed)
            embed = embed[:, :2]
