...
def analysis_spectral(**kwargs):
    ...
    _run("voxelium_apex.analysis.spectral", kwargs)
...
def analysis_star_subset(**kwargs):
    ...
    _run("voxelium_apex.analysis.star_file_subset", kwargs)


@analysis.command("extract", context_settings=_CONTEXT_SETTINGS, no_args_is_help=True)
@_options([
    click.option("--box-size", '-bs', type=int, required=True,
                 help="Box size of the extracted subtomograms, in pixels."),
    click.option("--tomograms", '-t', type=click.Path(exists=True, dir_okay=False, path_type=Path),
                 required=True, help="Path to the tomograms.star file (one entry per tiltseries)."),
    ...
    # --- input source: exactly one of the two ---
    click.option("--particles", '-p', type=click.Path(exists=True, dir_okay=False, path_type=Path),
                 default=None, help="Path to a RELION particles.star file. Mutually exclusive with --picks-uri."),
    ...
    # --- shared geometry ---
    click.option("--tiltseries-relative-dir", '-trd', type=click.Path(path_type=Path), default=Path("./"),
                 show_default=True, help="Directory the tiltseries paths in the star files are relative to. (not needed if absolute paths are used in the starfile or the tiltseries are in the tomograms.star file)"),
    click.option("--crop-size", '-cs', type=int, default=None,
                 help="Crop size in pixels. If omitted, defaults to the box size."),
    click.option("--bin", '-b', type=int, default=1, show_default=True, help="Binning factor for the subtomograms."),
    click.option("--debug", '-d', is_flag=True, default=False, help="Enable debug logging."),
])
def analysis_extract(**kwargs):
    ...
    _run("voxelium_apex.analysis.extract", kwargs)
...
@sha3d.command("train", context_settings=_CONTEXT_SETTINGS)
@_options([
    click.argument("log_dir", type=str),
    click.option("--input", "-i", type=str, default=None,
                 help="input job (job directory or optimizer-file)"),
    click.option("--particle_diameter", type=int, default=None,
                 help="size of circular mask (ang)"),
    click.option("--circular_mask_thickness", type=int, default=20,
    ...
    click.option("--gpu", type=str, default=None, help="gpu to use"),
    click.option("--checkpoint_time", type=int, default=10,
                 help="Minimum time in minutes between checkpoint saves"),
    click.option("--image_steps", type=int, default=500,
                 help="Log tensorboard images every n steps"),
    click.option("--stats_steps", type=int, default=100,
                 help="Log tensorboard statistics every n steps"),
    click.option("--max_steps", "--steps", type=int, default=int(1e9),
                 help="number of steps to train"),
    click.option("--max_train_epochs", "--epochs", type=int, default=10,
    ...
    click.option("--s_l1_weight", type=click.FloatRange(min=0), default=0,
                 help="S L1 loss weight"),
    click.option("--s_l2_weight", type=click.FloatRange(min=0), default=1e-4,
                 help="S L2 loss weight"),
    click.option("--z_compactness_weight", type=click.FloatRange(min=0), default=1e-4,
                 help="Compactness of the embedding"),
    click.option("--feature_bandpass", type=str, default=None,
    ...
    click.option("--z_encoder_dims", type=str, default="128,128,128",
                 help="Comma separated integers used for Z-encoder hidden layer dimensions."),
    click.option("--s_encoder_dims", type=str, default="128,128,128,128",
...
def sha3d_train(**kwargs):
    ...
    _run("voxelium_apex.sha3d.train", kwargs)
...
def sha3d_summary(**kwargs):
    ...
    _run("voxelium_apex.sha3d.summary", kwargs)


@sha3d.command("viewer", context_settings=_CONTEXT_SETTINGS)
@_options([
...
def sha3d_viewer(**kwargs):
    ...
    _run("voxelium_apex.sha3d.viewer", kwargs)
...
def sha3d_submit(**kwargs):
    ...
    _run("voxelium_apex.sha3d.job_submit", kwargs)
