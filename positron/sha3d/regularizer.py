...
class Regularizer(torch.nn.Module):
    ...
    @torch.no_grad()
    def update(self, x, y, ctf2, train_mask, valid_mask, momentum=0.99):
    ...
    def get_spectral_summary(self):
        ...
        return {
            'Means': [
                ...
                {'y': torch.clip(self.c_train_max_spectrum.data, 0, 1),
                 'label': 'cc train', 'color': 'blue', 'linestyle': 'dashed'},
                {'y': torch.clip(self.c_valid_max_spectrum.data, 0, 1),
                 'label': 'cc valid', 'color': 'green', 'linestyle': 'dashed'}
            ],
            'Powers': [
            ...
            'x': [
                {'y': self.get_x_signal(), 'label': 'signal', 'color': 'green'},
                {'y': self.get_x_noise(), 'label': 'noise', 'color': 'red'},
            ],
            'y Weights': [
                {'y': self.get_y_weight()},
            ],
